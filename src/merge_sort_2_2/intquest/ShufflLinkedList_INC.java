...
/**
 * @author deuru
 *     <p>Shuffling a linked list. Given a singly-linked list containing n items, rearrange the
 *     items uniformly at random. Your algorithm should consume a logarithmic (or constant) amount
 *     of extra memory and run in time proportional to nlogn in the worst case.
 */
public class ShufflLinkedList_INC {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < l; i++) list.add(i);

    String string = "sm ";
    System.out.println(string + "2" + "string");
    String string2 = "2";
    System.out.println("sm " + string2 + "string");

    System.out.println("sm " + "2" + "string");
    String string3 = "sm ";
    System.out.println(string3 + "2" + "string");

    String string4 = "List: ";
    System.out.println(string4 + list);
    // shuffle2(list);
  ...
  public static void bias(int times, LinkedList<Integer> list) {
    ...
    for (int i = 0; i < times; ++i) {
      ...
      counts.compute(l.stream().mapToInt(a -> a).toArray(), (k, v) -> v == null ? 1 : v + 1);
    }
    counts.entrySet().stream()
        ...
        .forEach(
            e ->
                System.out.printf(
                    "%s | times: %s | prob: %.3f\n",
                    Arrays.toString(e.getKey()), e.getValue(), (double) e.getValue() / times));
  }
  ...
  public static <T> void shuffleRd(LinkedList<T> l) {
    ...
    int s = l.size();
    LinkedList<T> l1 = new LinkedList<>(), l2 = new LinkedList<>();

    while (!l.isEmpty()) {
      l1.add(l.pollFirst());
      if (!l.isEmpty()) l2.add(l.pollFirst());
    }
  ...
  private static <T> void merge(
      LinkedList<T> l, LinkedList<T> l1, LinkedList<T> l2, boolean dummyNode) {
    ...
    while (!l1.isEmpty() || !l2.isEmpty()) {
      if (l1.isEmpty()) l.add(l2.pollFirst());
      else if (l2.isEmpty()) l.add(l1.pollFirst());
      else if (StdRandom.bernoulli()) l.add(l1.pollFirst());
      else l.add(l2.pollFirst());
  ...
  private static <T extends Comparable<T>> void merge(
      LinkedList<T> a, T[] aux, int lo, int mid, int hi, Iterator<T> cIter, LinkedList<T> list) {
    ...
    for (int i = lo; i <= hi; i++) {
      if (l > mid) list.add(aux[r++]);
      else if (r > hi) list.add(aux[l++]);
      else if (StdRandom.bernoulli()) list.add(aux[l++]);
  ...
  private static void merge(int[] arr, int[] aux, int lo, int mid, int hi) {
    ...
    int li = lo, ri = mid + 1;
    for (int i = lo; i <= hi; i++) {
      if (li > mid) arr[i] = aux[ri++];
      else if (ri > hi) arr[i] = aux[li++];
      else if (StdRandom.bernoulli()) arr[i] = aux[ri++];
      else arr[i] = aux[li++];
    }
  }
}
