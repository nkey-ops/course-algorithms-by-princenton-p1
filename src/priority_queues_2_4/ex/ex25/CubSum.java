 ...
 */
 ...
 public class CubSum {
       ...
       public CubSum(int sum, int i, int j) {
           this.sum = sum;
           this.i = i;
           this.j = j;
       }
     ...
     * Space Comlexity: O n 
     */
    private static CubSum[] sum(int n) {
        MinPQ<CubSum> heap = new MinPQ<>(
                (a, b) -> Integer.compare(a.sum, b.sum));

        CubSum[] arr = new CubSum[n * n];
        int in = 0;

        for (int i = 0; i < n; i++) {
            heap.insert(
                    new CubSum((int) Math.pow(i, 3), i, 0)); 
        }
        

        while(!heap.isEmpty()) {
           CubSum num =  heap.delMin();
           arr[in++] = num; 
     ...
     *
     */
    public static CubSum[] sumBrute(int n ) {
        CubSum[] arr = new CubSum[n * n];
